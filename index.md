...
<style>
    ...
    .liveweather-container{max-width:1100px;margin:0 auto;padding:0 16px}
    .liveweather-hero{display:block;margin:0 auto 1rem auto;max-width:420px;width:55%;min-width:240px;height:auto}

    #wx-dashboard{display:grid;gap:22px;margin-bottom:56px;}
    .liveweather-status{display:flex;justify-content:flex-end;align-items:center}
    #wx-conn{font-size:13px;color:var(--muted)}

    /* Center the two cards on wide screens */
    .liveweather-cards{display:grid;gap:22px;justify-content:center}
    @media (min-width:880px){
      .liveweather-cards{grid-template-columns:repeat(2,minmax(360px,520px))}
    }
    @media (max-width:879.98px){
      .liveweather-cards{grid-template-columns:minmax(280px,700px)}
    }

    .liveweather-card{position:relative;background:var(--card);border:1px solid var(--border);border-radius:14px;box-shadow:var(--shadow);padding:34px 22px 60px}
    .liveweather-card h2{margin:0 0 26px;text-align:center;font-weight:800;font-size:calc(clamp(18px,2.2vw,22px) * var(--title-scale))}

    .liveweather-card .gear{position:absolute;right:12px;bottom:12px}
    .liveweather-card .gear summary{list-style:none;cursor:pointer;width:32px;height:32px;border-radius:50%;display:grid;place-items:center;background:#fff;border:1px solid var(--border);box-shadow:0 2px 8px rgba(0,0,0,.06)}
    .liveweather-card .gear summary::-webkit-details-marker{display:none}
    .liveweather-card .gear[open] summary{box-shadow:0 6px 18px rgba(0,0,0,.12)}
    .liveweather-card .gear .panel{position:absolute;right:0;bottom:40px;min-width:290px;background:#fff;border:1px solid var(--border);border-radius:12px;box-shadow:0 16px 34px rgba(0,0,0,.16);padding:10px;display:grid;gap:10px}
    .liveweather-card .menu-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px 10px;font-size:13px}
    .liveweather-card .menu-grid label{display:flex;align-items:center;gap:6px}
    /* push Total rain selector to the right column */
    .liveweather-card .menu-grid label.total-right{grid-column:2}

    /* Conditions layout; “snake” order when items are hidden */
    .liveweather .cond-grid{   display:grid;   gap:32px 34px;   grid-template-columns:repeat(2,minmax(0,1fr));   grid-auto-flow:row dense; /* keeps 1–8 row-major and fills gaps */   margin-top:22px; }
    .liveweather .cond-item{direction:ltr}
    .liveweather .cond-item .label{font-weight:800;font-size:clamp(15px,1.9vw,19px); color:var(--title-ink)}
    .liveweather .cond-item .value{font-weight:800;letter-spacing:.2px;font-variant-numeric:tabular-nums;font-size:clamp(28px,4.4vw,40px)}
    .liveweather .value-temp{color:var(--temp)} .liveweather .value-dew{color:var(--dew)} .liveweather .value-rh{color:var(--rh)} .liveweather .value-slp{color:var(--slp)}
    .liveweather .value-rainrate{color:var(--rainrate)} .liveweather .value-raintoday{color:var(--raintotal)} .liveweather .value-solar{color:var(--solar)} .liveweather .value-uv{color:var(--uv)}
    .liveweather .asof{position:absolute;left:22px;bottom:14px;color:var(--muted);font-size:12px}
    .liveweather .hidden{display:none!important}

/* Wind — original cosmetics */
.liveweather .wind-wrap{display:grid;grid-template-columns:1fr;justify-items:center;gap:14px}
.liveweather .wind-main{ text-align:center; }
.liveweather .wind-speed{
  font-weight:900;
  font-size:clamp(40px,6.6vw,60px);
  line-height:1.05;
  color:#2563eb;
}
.liveweather .wind-cardinal{
  margin-top:8px;
  font-weight:800;
  font-size:18px;
  color:#64748b;
}
.liveweather .wind-line{ margin-top:16px; font-size:20px; }
.liveweather .wind-line .label{ font-weight:800; color:#111; }
.liveweather .wind-number{ color:#2563eb; font-weight:800; }


    /* Compass */
    .liveweather .compass{width:100%;max-width:340px;min-width:220px;aspect-ratio:1/1;overflow:visible;}
    .liveweather .ring{fill:none;stroke:#dfe5ee;stroke-width:2}
    .liveweather .tick{stroke:#cdd3dc;stroke-width:1}
    .liveweather .tick.major{stroke:#b7bfca;stroke-width:1.4}
    .liveweather .labels{fill:#4a5568;font-size:6px;font-weight:700}
    .liveweather .needle{fill:var(--needle);stroke:var(--needle)}
    #wx-needle,#wx-needle2{transform-box:view-box;transform-origin:50px 50px;transition:transform 140ms ease-out}

    /* README area */
    .liveweather .markdown-body{box-sizing:border-box;padding:0}
    .liveweather #app{ margin-top:48px; }

/* Phones */
@media (max-width: 480px){
  .liveweather-container{margin:10px auto 20px}
  .liveweather-hero{max-width:170px;width:42%;margin:.2rem auto .6rem}
  .liveweather #wx-dashboard{gap:14px;margin-bottom:44px;}
  .liveweather-cards{gap:14px}
  .liveweather-card{padding:16px 12px 42px}
  .liveweather-card h2{font-size:calc(clamp(14px,4.0vw,18px) * 1.06)}
  .liveweather .cond-grid{gap:16px 18px}
  .liveweather .cond-item .label{font-size:clamp(12px,3.4vw,14px)}
  .liveweather .cond-item .value{font-size:clamp(20px,5.6vw,26px)}
  .liveweather .asof{left:12px;bottom:10px;font-size:11px}
  .liveweather .compass{max-width:220px;min-width:170px}
  .liveweather .wind-values{max-width:300px}
  .liveweather .wind-values .speed{font-size:clamp(20px,6.2vw,28px)}
}
...
@media screen and (orientation: landscape) and (max-height: 500px) {
  ...
  .liveweather-container { padding: 0 10px; }     /* a bit more edge room */
}

  </style>

  <div class="liveweather">
    <div class="liveweather-container">
      <img class="liveweather-hero" src="/Images/WxManBran_Logo.png" alt="Two red hurricane warning flags" loading="lazy">

      <main id="wx-dashboard" aria-label="Live weather dashboard">
        <header class="liveweather-status"><div id="wx-conn">Status: Offline</div></header>

        <section class="liveweather-cards">
        <!-- Conditions -->
        <article class="liveweather-card" aria-label="Current Conditions">
          <h2>Conditions</h2>
        ...
        <!-- Wind -->
        <article class="liveweather-card" aria-label="Wind">
          <h2>Wind</h2>
          <div class="wind-wrap">
