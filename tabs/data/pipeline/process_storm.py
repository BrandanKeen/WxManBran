...
DATASETS_DIR = TABS_DIR / "data" / "datasets"
PIPELINE_DIR = Path(__file__).resolve().parent
STORMS_DIR = TABS_DIR / "previous-storms" / "_storms"
...
def find_notebook(slug: str) -> Path:
    nb_dir = DATASETS_DIR / slug / "notebooks"
...
def run_parse(notebook: Path, spec: Path) -> None:
    ...
    result = subprocess.run(
        [
            sys.executable,
            str(PIPELINE_DIR / "parse_notebook_plots.py"),
            str(notebook),
        ],
        capture_output=True,
        text=True,
...
def run_build(csv_path: Path, spec: Path, public_dir: Path) -> None:
    ...
    subprocess.run(
        [
            sys.executable,
            str(PIPELINE_DIR / "build_interactive_from_spec.py"),
            "--csv",
            str(csv_path),
            "--spec",
            str(spec),
...
def run_build_static(csv_path: Path, spec: Path, public_dir: Path) -> None:
    ...
    subprocess.run(
        [
            sys.executable,
            str(PIPELINE_DIR / "build_static_from_spec.py"),
            "--csv",
            str(csv_path),
            "--spec",
            str(spec),
...
def run_embed(spec: Path, storm_md: Path, public_dir: Path) -> None:
    subprocess.run(
        [
            sys.executable,
            str(PIPELINE_DIR / "embed_plots_in_storm_page.py"),
            "--spec",
            str(spec),
            "--storm-md",
            str(storm_md),
            "--public-dir",
            str(public_dir.relative_to(REPO_ROOT)),
        ],
        check=True,
    )
...
def process_storm(slug: str) -> None:
    ...
    spec_path = DATASETS_DIR / slug / "specs" / f"{slug}.json"
    public_dir = DATASETS_DIR / slug / "plots"
