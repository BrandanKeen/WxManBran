    ...
    Plotly.newPlot('chart', figure.data, figure.layout, config).then((gd) => {{
      ...
      function setupTouchHover() {{
        ...
        const refreshTouchTargets = () => {{
          ...
          const plots = gd._fullLayout._plots || {{}};
          touchTargets = Array.from(root.querySelectorAll('rect.nsewdrag'))
            .map((node) => {{
              const subplot = node.getAttribute('data-subplot');
              if (!subplot) {{
                return null;
              }}
              const plot = plots[subplot];
              if (!plot || !plot.xaxis) {{
                return null;
              }}
              return {{ node, axis: plot.xaxis }};
            }})
